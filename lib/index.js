...
micro.modules = require('./modules');

micro.procedures = [];
micro.hemera_add_array = [];

micro.loadModules = (procMod) => {
  const modules = {};
  if(procMod.load) {
    const loadArray = JSON.parse( JSON.stringify(procMod.load) );
...
micro.create = (env=null) => {
  if(env!=null) micro.env = Object.assign({}, micro.env, env); // apply env merge

  // Add other modules
  micro.modules.env = () => micro.env; // create env module injector
  //micro.modules.debug = () => micro.debug; // create debug module
  micro.modules.add = () => micro.add; // create add module
  micro.modules.act = () => micro.act; // create act module

  // Start Hemera
  const natsCon = nats.connect({ url: micro.env.nats_url, user: micro.env.nats_user, pass: micro.env.nats_pass }); // create nats connection
  const hemeraConf = { logLevel: micro.env.hemera_logLevel, timeout: micro.env.hemera_timeout }; // config hemera
  micro.hemera = new Hemera( natsCon, hemeraConf); // create hemera instance
  return micro;
};

micro.addProcedure = (proc) => {
  proc.load = proc.load || [];
  proc = micro.loadModules(proc);
  micro.procedures.push(proc);
  return micro;
}
...
micro.add = (action, _func=null) => {
  const func = _func == null ? null : (req, cb) => _func(req.payload, cb);
  micro.hemera_add_array.push({action, func});
  return micro;
};

micro.act = (action, payload, cb=null) => {
  if(cb==null) { cb = payload; payload = null; }
  micro.hemera.act({topic: micro.env.app_topic, cmd: action, payload}, cb);
  return micro;
};

micro.start = (main=null) => {
  micro.hemera.ready(async () => {
    for(let add_line of micro.hemera_add_array)
      micro.hemera.add({topic: micro.env.app_topic, cmd: add_line.action, timeout$: micro.env.hemera_timeout, expectedMessages$: 1}, add_line.func);
    if(main) main();
  });
  return micro;
};

micro.close = (...args) => micro.hemera.close(...args)
