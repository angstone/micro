...
function Micro(_env) {
  ...
  this.when_online_should_runs = [];
  this.all_when_online_functions_ready = false;
  ...
  this.loadModules = (procMod) => {
    ...
    if(procMod.load) {
      ...
      if( loadArray instanceof Array ) {
        ...
        if(procMod.whenOnline) {
          procMod.when_online_ready = false;
          this.when_online_should_runs.push(procMod);
        }
      }
    }
    return procMod;
  ...
  // Add singleton module loader
  this.modules.loader = () => this.loadModules;
  ...
  this.grantAllWhenOnlineReady = (resolve)=>{
    if(!this.all_when_online_functions_ready) {
      this.when_online_should_runs.forEach(module=>{
        if(!module.when_online_ready) return setTimeout(()=>{
          this.grantAllWhenOnlineReady(resolve);
        }, 30);
      });
      if(!this.all_when_online_functions_ready) {
        this.all_when_online_functions_ready = true;
        resolve();
      }
    }
  };

  this.runWhenOnlineFunctions = (resolve)=>{
    this.when_online_should_runs.forEach(module=>{
      module.whenOnline().then(()=>{
        module.when_online_ready = true;
      });
    });
    this.grantAllWhenOnlineReady(resolve);
  };
