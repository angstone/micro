...
function Evt() {
  ...
  this.send = async function(type, payload=null, stream=null) {
    ...
    if(response.status==201)
      return parseInt( response.headers.get('location').split("/").slice(-1)[0] , 10); // The Event Number
    else {
  ...
  this.get = async function(event_number, stream=null) {
    ...
    return fetch(url, {method, headers}).then(res => res.status==404 ? null : res.json())
  }
};
