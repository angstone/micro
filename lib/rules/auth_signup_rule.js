...
rule.respond = function(eventNumber, payload) {
  return new Promise(resolve=>{
    //console.log('GOT EMISSION: ', eventNumber)
    let user = this.models.user.data({id:eventNumber}).first();
    if(payload.login==user.login && payload.name==user.name) resolve(user.id);
    else throw this.load.error.is('BAD REDUCING','The command auth signup was not reduced OK');
  });
}
