  ...
  it('Should be able to provide a rule object', function(done) {
    micro.addProcedure({
      ...
      start: async function() {
        ...
        const rule_loaded = await this.load.ruler.loadRule(require('../lib/rules/auth_signup_rule'));
        expect(rule_loaded).to.be.exists()
        expect(rule_loaded.pre_validation).to.be.exists()
        expect(rule_loaded.models.user).to.be.exists()
        expect(rule_loaded.models.user.SYSTEM_AGENT).to.be.equals(require('../lib/models/user_model').SYSTEM_AGENT)
        done()
      }
    }).start();
  })
